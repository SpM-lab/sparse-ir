...
Note: these tests deliberately do *not* wrap their bodies in
``try/except: pytest.skip(...)``.  A failure here is a defect, not an
unsupported precondition, and swallowing it makes the suite report success
...
class TestConsistencyChecks:
    ...
    @pytest.mark.parametrize("lambda_val", [10, 42])
    def test_singular_value_ordering(self, statistics, lambda_val):
        ...
        assert np.all(s[:-1] >= s[1:]), \
            f"Singular values not decreasing for {statistics}, λ={lambda_val}"

        # Check positivity
        assert np.all(s > 0), \
            f"Non-positive singular values for {statistics}, λ={lambda_val}"

        # Check normalization (first should be largest)
        assert s[0] == np.max(s), \
            f"First singular value not largest for {statistics}, λ={lambda_val}"
...
class TestMemoryAndPerformance:
    ...
    def test_repeated_operations(self, rng):
        ...
        for _ in range(10):
            ...
            error = np.max(np.abs(al_test - al_recovered))
            assert error < 1e-12, \
                f"Accuracy degraded after repeated operations: {error}"
